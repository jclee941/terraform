    ...
    mcphub = {
      ...
      cloud_init = {
        ...
        runcmd = [
          ...
          "grep -q oc-dev /etc/fstab || echo 'jclee@${module.hosts.hosts.oc.ip}:/home/jclee/dev /mnt/oc-dev fuse.sshfs _netdev,allow_other,default_permissions,IdentityFile=/root/.ssh/id_rsa,reconnect,ServerAliveInterval=15,ServerAliveCountMax=3 0 0' >> /etc/fstab",
          "grep -q oc-kratos /etc/fstab || echo 'jclee@${module.hosts.hosts.oc.ip}:/home/jclee/.kratos /mnt/oc-kratos fuse.sshfs _netdev,allow_other,default_permissions,IdentityFile=/root/.ssh/id_rsa,reconnect,ServerAliveInterval=15,ServerAliveCountMax=3 0 0' >> /etc/fstab",
