...
- `terraform/`: Standalone Grafana provider workspace. Split into: `main.tf` (folders, data sources, dashboards), `contact_points.tf` (2 contact points: alert-log-fallback, slack-alerts), `notification_policy.tf` (default policy with Slack + fallback routing), `alerting_locals.tf` (ES + Prometheus alert rule definitions), `alerting_rules.tf` (3 rule groups: homelab_logs, infrastructure_health, mcp_alerts — 14 rules total), `service_accounts.tf` (2 service accounts + tokens).
...
- **Alert Routing**: Alerts route to Slack (critical/warning) and alert-log-fallback (info). No n8n webhook integration.
