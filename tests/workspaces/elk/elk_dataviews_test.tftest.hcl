...
run "critical_index_template_patterns_derived_from_registry" {
  ...
  assert {
    condition     = length(module.index_templates["logs_critical"].index_patterns) == 2
    error_message = "Critical template should have 2 patterns (elk, pve)"
  }

  assert {
    condition     = contains(module.index_templates["logs_critical"].index_patterns, "logs-elk-*")
    error_message = "Critical template should include logs-elk-*"
  }

  assert {
    condition     = contains(module.index_templates["logs_critical"].index_patterns, "logs-pve-*")
...
run "ephemeral_index_template_patterns_derived_from_registry" {
  ...
  assert {
    condition     = length(module.index_templates["logs_ephemeral"].index_patterns) == 2
    error_message = "Ephemeral template should have 2 patterns (github-runner, youtube)"
  }

  assert {
    condition     = contains(module.index_templates["logs_ephemeral"].index_patterns, "logs-github-runner-*")
    error_message = "Ephemeral template should include logs-github-runner-*"
  }

  assert {
    condition     = contains(module.index_templates["logs_ephemeral"].index_patterns, "logs-youtube-*")
...
run "index_template_priority_ordering" {
  ...
  assert {
    condition     = module.index_templates["logs_critical"].priority == 300
    error_message = "Critical template should have highest priority (300)"
  }

  assert {
    condition     = module.index_templates["logs_ephemeral"].priority == 250
    error_message = "Ephemeral template should have medium priority (250)"
  }

  assert {
    condition     = module.index_templates["logs"].priority == 200
    error_message = "Standard catch-all template should have lowest priority (200)"
  }

  assert {
    condition     = contains(module.index_templates["logs"].index_patterns, "logs-*") && length(module.index_templates["logs"].index_patterns) == 1
