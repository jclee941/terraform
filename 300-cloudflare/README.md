# 300-cloudflare: Cloudflare Infrastructure & Secrets Management
...
- **Secret metadata registry**: `inventory/secrets.yaml` (50+ entries, no values)
- **Terraform workspace**: `*.tf` files in this directory
- **1Password secrets**: `homelab` vault (consumed via `onepassword-secrets` module)
- **Worker code**: `workers/synology-proxy/`
...
- `gh` CLI (GitHub authentication completed)
- `vault` CLI (when using Vault)

### Setup

```bash
cp .env.example .env
# Enter actual authentication credentials in .env:
...
### Collect Secret Values (local project → tfvars)

```bash
# Dry run (preview)
go run scripts/collect.go --dry-run

# Run (creates secret-values.tfvars)
...
```bash
go run scripts/audit.go               # Validate all targets
go run scripts/audit.go --help         # Usage
```

### Generate Worker Bindings

```bash
go run scripts/generate-bindings.go                # TOML (default)
...
1. Add metadata to `inventory/secrets.yaml`:
   ```yaml
   - name: NEW_SECRET_NAME
     ...
     sensitive: true
   ```
2. Re-run `go run scripts/collect.go` → `terraform apply`

## Safety Notes

- `secrets.yaml` stores **no values** (metadata only).
- `secret-values.tfvars` is included in `.gitignore`.
- `.env` file is included in `.gitignore`.
- Terraform state contains sensitive values. A **remote backend is recommended**.
- Never commit `.tfvars`, `.env`, or `data/` output files.
- Apply via CI only. Local `make apply` is disabled.
