...
resource "cloudflare_ruleset" "waf_custom" {
  ...
  rules = [
    ...
    # Rule 2: Block suspicious paths
    {
      ...
      expression  = "not http.request.uri.path within {\"/\" \"/kibana\" \"/elasticsearch\" \"/api\" \"/mcphub\"} and (http.request.uri.path contains \"/.env\" or http.request.uri.path contains \"/wp-admin\" or http.request.uri.path contains \"/wp-login.php\" or http.request.uri.path contains \"/xmlrpc.php\" or http.request.uri.path contains \"/phpmyadmin\" or http.request.uri.path contains \"/.git\" or http.request.uri.path contains \"/config.json\" or http.request.uri.path contains \"/actuator\")"
      action      = "block"
      enabled     = true
    },
